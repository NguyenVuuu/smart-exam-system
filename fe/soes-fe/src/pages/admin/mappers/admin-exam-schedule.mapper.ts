...
export const toAdminSchedule = (row: ExamScheduleApiDto): AdminExamSchedule => ({
  ...
  allowStudentReview: row.reviewPolicy !== 'NONE',
  enableTabLock: row.enableTabLock ?? true,
  maxTabSwitches: row.maxTabSwitches ?? 3,
  requireFullscreen: row.requireFullscreen,
  enableWebcam: row.enableWebcam,
  enableScreenMonitoring: row.enableScreenMonitoring ?? false,
...
export function toSchedulePayload(schedule: AdminExamSchedule, existing: boolean): SchedulePayload {
  ...
  return {
    ...
    maxTabSwitches: schedule.enableTabLock ? (schedule.maxTabSwitches ?? 3) : null,
    requireFullscreen: schedule.requireFullscreen ?? true,
    enableWebcam: schedule.enableWebcam ?? true,
    enableScreenMonitoring: schedule.enableScreenMonitoring ?? false,
    blockCopyPaste: schedule.blockCopyPaste ?? true,
