...
export const createTeacherMakeupSchedule = (
  examId: string,
  scheduleId: string,
  payload: TeacherExamSchedulePayload & { studentIds: string[] },
) => apiClient.post<ApiResponse<TeacherExamScheduleDto>>(
  `/teacher/exams/${examId}/schedules/${scheduleId}/makeup`,
  payload,
).then(({ data }) => data.data)
