...
interface Props {
  ...
  onUpdate: (id: string, payload: { title: string; content: string }) => Promise<void>
  onPin: (id: string, pinned: boolean) => Promise<void>
  onDelete: (id: string) => Promise<void>
}

export default function CourseTimelineTab({ announcements, onCreate, onUpdate, onPin, onDelete }: Props) {
        ...
        {[...announcements].sort((a, b) => Number(Boolean(b.pinned)) - Number(Boolean(a.pinned))).map((post) => (
          <article key={post.id} className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            ...
            {!!post.attachedFiles?.length && <div className="mt-4 flex flex-wrap gap-2">{post.attachedFiles.map((file) => (
              <button key={file.name} className="inline-flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-sm text-gray-700"><Paperclip size={15} /><span>{file.name}</span><span className="text-gray-400">({file.size})</span><Download size={14} /></button>
            ))}</div>}
          </article>
        ))}
      </div>
