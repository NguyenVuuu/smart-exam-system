...
export function ExamProctoringTab({
  ...
  const columns: ColumnDef<ViolationRecord>[] = [
    ...
    },
    {
      ...
      width: "160px",
      render: (v) => (
        <span className="text-gray-600 text-sm font-medium">{v.timestamp}</span>
      ),
    },
    {
      header: "Thời Lượng",
    ...
    },
    {
      ...
      render: (v) => (
        <AppBadge
          tone={violationSeverityTone[v.severity]}
          ...
          <div className="divide-y divide-gray-100">
            {proctoringSessions.map((session) => (
              <div
                ...
                className="grid gap-3 px-5 py-4 md:grid-cols-[minmax(180px,1fr)_140px_150px_150px_minmax(180px,1fr)] md:items-center"
              >
                ...
                <AppBadge
                  tone={session.isOnline ? "blue" : "gray"}
                  shape="rounded"
                  className="w-fit px-2.5 py-1 text-xs font-semibold"
                >
                  {session.isOnline ? "Online" : "Offline"}
                </AppBadge>
                <AppBadge
                  tone={webcamStatusTone[session.webcamStatus]}
                  shape="rounded"
                  className="w-fit px-2.5 py-1 text-xs font-semibold"
                >
                ...
                <span className="text-sm text-gray-500">
                  {session.lastWebcamHeartbeatAt
                    ? formatCompactTime(session.lastWebcamHeartbeatAt)
                    : "-"}
                </span>
                <span className="text-sm text-gray-500">
                  {session.lastViolation
                    ? formatProctoringLastViolation(session.lastViolation)
                    : "Chưa có vi phạm"}
                </span>
              </div>
            ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <DataTable
          ...
          data={violations.filter(
            (violation) => violation.scheduleId === selectedSession?.id,
          )}
          keyExtractor={(v) => v.id}
...
function formatProctoringLastViolation(
  violation: NonNullable<ProctoringSessionRecord["lastViolation"]>,
) {
  const duration =
    violation.durationSeconds === null && violation.endedAt === null
      ? "Đang diễn ra"
