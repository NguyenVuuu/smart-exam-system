...
async function validate(data: CourseOfferingBody, currentId?: string) {
  ...
  if (data.status === 'ACTIVE' && (semester.status === 'CLOSED' || subject.status !== 'ACTIVE' || teacher.status !== 'ACTIVE')) {
    throw new ValidationError('Active course offering requires an open semester, active subject, and active teacher')
  }
