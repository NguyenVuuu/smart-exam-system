...
export const uploadMaterials = async (req: Request, res: Response) => {
  ...
  send(res, await service.uploadMaterials(req.user!.profileId, id, uploadedFiles(req)), 201)
}
const materialParams = z.object({ id: z.string().min(1), materialId: z.string().min(1) })

export const downloadMaterial = async (req: Request, res: Response) => {
  const { id, materialId } = materialParams.parse(req.params)
  const file = await service.downloadMaterial(req.user!.profileId, id, materialId)
...
export const createPost = async (req: Request, res: Response) => {
  ...
  send(res, await postService.create(req.user!.profileId, id, postBodySchema.parse(req.body), uploadedFiles(req)))
}

export const updatePost = async (req: Request, res: Response) => {
  const { id, postId } = postParams.parse(req.params)
  send(res, await postService.update(req.user!.profileId, id, postId!, postBodySchema.parse(req.body), uploadedFiles(req)))
}
